...
class PurchaseOrderRepairWizard(models.TransientModel):
    _name = 'purchase.order.repair.wizard'
    _description = 'Generate Purchase Order from Repair Order'
    ...
    order_line_ids = fields.One2many('purchase.order.line.repair.wizard', 'order_id', string='Order Lines', copy=True)
    repair_order_id = fields.Many2one('mrp.repair', string="Repair Order", required=True,
                                    help="Not empty if an origin for purchase order was repair order")
    ...
    def create_purchase_orders(self):
        ...
        for ven in vendor_ids:
            ...
            repair_lines = self.order_line_ids.filtered(lambda l: l.partner_id.id == ven and l.product_qty_to_order > 0)
            if not len(repair_lines):
                continue
            ...
            if len(p_order):
                ...
                p_order = po_env.create({
                    ...
                    'repair_order_id': self.repair_order_id.id,
                    'customer': self.partner_id.id,
                    'salesperson': self.salesperson_id.id,
                    'order_line': False,
                })
    ...
    def create_po_lines_from_so(self, s_order):
        for line in s_order.operations:
            ...
            p_line = self.env['purchase.order.line.repair.wizard'].create({
                'name': line.name,
...
class PurchaseOrderLineWizard(models.TransientModel):
    _name = 'purchase.order.line.repair.wizard'
    ...
    order_id = fields.Many2one('purchase.order.repair.wizard', string='Order Reference', index=True, required=True, ondelete='cascade')
